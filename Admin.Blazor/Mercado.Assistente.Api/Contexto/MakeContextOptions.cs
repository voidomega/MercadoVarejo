...
namespace Ma.MercadoAssistente.Api.Contexto
{
        ...
        public static void AddDbContextPool(IServiceCollection services)
        {
            ...
            if (ServerType == "PostGreSql")
            {
                if (!optionsBuilder.IsConfigured)
                    services.AddDbContextPool<MaContext>(options => options.UseNpgsql(finalConStr));
               
            }
            else if (ServerType == "MySql")
            {
                if (!optionsBuilder.IsConfigured)
                    services.AddDbContextPool<MaContext>(options => options.UseMySql(finalConStr));
            }
            else if (ServerType == "SqlSrv")
            {
                if (!optionsBuilder.IsConfigured)
                    services.AddDbContextPool<MaContext>(options => options.UseSqlServer(finalConStr));
